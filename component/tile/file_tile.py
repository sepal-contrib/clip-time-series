...
class FileTile(sw.Tile):
    ...
    def _load_file(self, widget, event, data):
        ...
        # check the variables 
        if not self.alert.check_input(file, cm.table.not_a_file): return
        if not self.alert.check_input(lat, cm.table.missing_input): return
        if not self.alert.check_input(lng, cm.table.missing_input): return
        if not self.alert.check_input(id_, cm.table.missing_input): return
        ...
        gdf = gdp.GeoDataFrame(
            df, 
            geometry=gdp.points_from_xy(df.lng, df.lat), 
            crs='EPSG:4326'
        )
