...
class InputTile(sw.Tile):
    ...
    def _display_data(self, widget, event, data):
        ...
        # check input
        if not self.alert.check_input(driver, cm.viz.no_driver): return
        if not self.alert.check_input(file, cm.viz.no_pts): return
        if not self.alert.check_input(bands, cm.viz.no_bands): return
        if not self.alert.check_input(start, cm.viz.no_start): return
        if not self.alert.check_input(end, cm.viz.no_end): return
        if not self.alert.check_input(square_size, cm.viz.no_square): return
        if not self.alert.check_input(image_size, cm.viz.no_image): return
        if start > end:
            raise Exception(cm.viz.wrong_date)
        
        # test specific to drivers
        if driver == 'planet':
            if not self.alert.check_input(planet_key, cm.viz.no_key): return
            if not self.alert.check_input(semester, cm.viz.no_semester): return
        elif driver == 'gee':
            if not self.alert.check_input(sources, cm.viz.no_sources): return
        ...
        msg = cs.set_msg(
            pts, bands, sources, Path(file).stem, 
            start, end, image_size, square_size, driver
        )
